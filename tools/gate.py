...
pytest/cocotb summary lines found in the captured output.
"""
...
def run_command(job_dir: Path, index: int, cmd):
    ...
    return {"argv": cmd, "exit_code": proc.returncode, "elapsed_s": round(time.perf_counter() - t0, 2),
            "log": str(log.relative_to(ROOT)), "tests": tests}
...
def main() -> int:
    ...
    print(f"[gate] {job}: {status} ({summary['elapsed_s']}s) -> {job_dir.relative_to(ROOT)}/summary.json")
