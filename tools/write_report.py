...
def facts(impl, dec, qs, manifest) -> list:
    ...
    out.append(f"**Verification.** {total:,} complete-core decisions across {len(dec)} hardware configurations match the "
               f"Python literal-descent reference (`results/decisions/*.csv`). Three 250-piece RTL games per depth-one "
               f"configuration also match Python at every move (`results/replays/`).")
    ...
    if cid in dec and cid in impl:
        ...
        out.append(f"**Latency.** The corpus median is {med:.0f} core cycles per decision (A0 serial: {a0:.0f}). The routed "
                   f"projection is {proj}. It divides simulated cycles by a clock constraint and is not a board measurement.")
    return out
...
def block(impl, dec, qs, manifest, tourn, disagree) -> str:
    ...
    out.append(f"Sources: `results/implementation.csv` contains {manifest.get('summary', {}).get('routing_attempts', 0)} routing attempts, "
               f"with {manifest.get('summary', {}).get('timing_met', 0)} meeting timing. The target is an ECP5 LFE5U-85F CABGA381 speed 6 at "
               f"50 MHz across seeds 1–5. Decisions come from `results/decisions/`, and policy results come from "
               f"`results/quality_summary.json`. RTL hash `{manifest.get('rtl_hash', '?')}`; toolchain `{manifest.get('toolchain_id', '?')}`.")
    out.append("")
    ...
    for cid, label in LABELS.items():
        ...
        med = f"{statistics.median(cyc):.0f}" if cyc else "n/a"
        mx = f"{max(cyc)}" if cyc else "n/a"
        n = f"{len(cyc)}/{len(cyc)} match" if cyc else "n/a"
    ...
    if proj:
        out.append("Projected decision latency uses median cycles ÷ 50 MHz. It is model-based and is not a board measurement. " + ". ".join(proj) + ".")
        out.append("")
    pr = qs.get("precision", {})
    if pr.get("policies"):
    ...
    if dp.get("policies"):
        ...
        for key, v in dp["policies"].items():
            ...
            cyc = [int(r["core_cycles"]) for r in dec.get(cid, [])]
            cyc_txt = f"{statistics.median(cyc):.0f}" if cyc else "n/a"
...
def main() -> int:
    ...
    text = block(*data)
    bad = 0
    # Since U19, the generated v1 tables live in docs/results.md and docs/design.md.
    # The README keeps a short first screen and links here.
