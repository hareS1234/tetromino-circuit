...
|---|---|---|---|
| `fast` | every push / pull request | Python-only: `make doctor-python`, generated-geometry and stream determinism, `make test-python` (JUnit XML), `make check-v1-results`, report check, workflow validation (`tools/ci_local.py --validate`) | 20 min |
| `hdl` | every push / pull request | the Linux HDL gate: pinned OSS CAD Suite, `make doctor`, `make test-bootstrap`, `make smoke`, `make test-rtl`, `make test-core ARCH=1 BOARD_REPR=1 COUNT=200 DRIVER=native`, `make test-identities`, `make test-result-schemas`, `make upgrade-smoke` (one real route, 50-state native corpus, 12-game v2 quality smoke), `make matrix-status` | 90 min |
...
evidence); U20 must push the branch to the intended repository, inspect the run, fix concrete
failures, pin the action SHAs, and record the run URL, commit SHA, conclusion and artifacts. The
first cold-cache run's observed duration is the budget reference; the `timeout-minutes` values
above are initial allowances, not measurements.
