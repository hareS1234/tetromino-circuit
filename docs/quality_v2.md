...
`model/longrun.py::play_summary` keeps the board, stream index, totals, trajectory hash, and random
policy state. It writes an atomic checkpoint every 1,000 locked pieces and at completion. Resume
requires an exact match across the quality key, protocol, policy, stream, cap, and model closure.

Summary mode and per-move replay mode make the same decisions. Tests compare trajectory hashes,
totals, and terminal reasons for P0, P7, and the random policy. They also interrupt and resume a game
at a checkpoint boundary. Corrupt current checkpoints fall back to the retained predecessor.
...
recorded as `<key>.failed.json` and counted as a failed job. The analysis refuses to summarize while
one exists. Progress prints at most every 60 s (jobs
...
Event time `T` counts locked pieces before the first top-out decision. A game stopped at cap `C`
reports `event_observed = false`. With a common cap and complete observations, the mean duration
equals `Σ_{t<C} S(t)` for product-limit survival `S(t) = P(T > t)`. The analysis asserts this identity
with exact rationals for every policy. Unit tests cover immediate and later events, censoring, ties,
and mixed rows. The median is reported only if `S` reaches 1/2 within the horizon, otherwise
...
`make bench-v2 MODE=pilot SUITE=pilot50k` ran P0 and P1 for 40 games in 107 seconds. P0 had a
restricted mean of 10,462 pieces, with every game topping out. P1 reached 27,819 pieces and hit the
cap in 6 of 20 games. Against P0, its paired gain was 17,357 pieces (CI95 [9,855, 24,689]) and 6,947
lines ([3,977, 9,807]). P1 led on 16 of 20 streams. About 8,000–10,000 decisions per second per policy; a 50,000-piece game
...
* **P1 (64, 64, 32, 16) plays about twice as long as the exact baseline (76, 51, 36, 18)** on these
  ...
  streams. No coefficient search or tuning used these results.
* **P5 (four-bit magnitudes) is indistinguishable from P0**: 30 of 100 games are move-for-move
  ...
  common-state disagreement rate of U14 translates into no measurable outcome difference here:   an inconclusive comparison, kept as such.
...
* This study covers depth one. The v1 depth results remain historical (`docs/design.md`). No i.i.d.
  suite was run. The held-out streams were never used for any development decision; the
