...
Generated by `tools/write_report_v2.py` from the committed result files; do not edit by hand (`make check-report-v2` regenerates and compares). Measurement definitions used throughout: **cycles** are RTL-simulation counts under the documented request/response protocol (Verilator, native driver); **routed fmax** is nextpnr-ecp5's reported maximum frequency for the routed netlist on the LFE5U-85F device model with auto-allocated I/O — a model-based figure, never a board measurement; a **projection** divides median cycles by a clock constraint that every seed met; **host wall time** is the runner process's wall clock; **restricted mean** and **survival** follow the censoring definitions of `docs/quality_v2.md` §3. Every table names its raw source.

## 1. Hardware matrix v2 (`benchmarks/hardware_v2.json`, U17)

ECP5 LFE5U-85F CABGA381 speed 6, Yosys `synth_ecp5 -nodsp`, nextpnr-ecp5 with `--freq` as the clock constraint and auto-allocated I/O; toolchain `oss-cad-suite-2026-09-04`; one synthesis per configuration reused for every target and seed; route budget 600 s (a1-cache-d1-p0-l4 1200 s). Raw: one `route-record-v2` per job under `results/v2/raw/routes/<route_key>.json`; derived CSV `results/v2/summary/routes_hardware-v2.csv` (current attempts only are summarised here; every attempt is in the CSV). Statuses: met = routed and the report passes the constraint, failed = routed but the reported fmax is below the constraint, timeout = the budget was exhausted (an observed resource limit, not proof the design cannot route). Counts: {'completed': 84, 'reused': 0, 'successes': 44, 'failures': 32, 'timeouts': 8, 'errors': 0}.
...
| A1 cache, P1 powers_of_two (`a1-cache-d1-p1-l1`) | 3941 | 2130 | 324 | 3/3 met; fmax 66.0–66.6 | — | — | — |
| A1 cache, P5 coeff_u4 (`a1-cache-d1-p5-l1`) | 3909 | 2112 | 332 | 3/3 met; fmax 66.4–68.3 | — | — | — |
| A1 cache, P6 coeff_u3 (`a1-cache-d1-p6-l1`) | 3882 | 2100 | 329 | 3/3 met; fmax 65.1–65.6 | — | — | — |
| A1 cache, P7 coeff_u2 (`a1-cache-d1-p7-l1`) | 3883 | 2094 | 322 | 3/3 met; fmax 65.9–68.5 | — | — | — |
...
The projection is RTL-simulation cycles divided by a routed clock constraint on a device model; it is model-based, not a board measurement, and is withheld where any of the three seeds did not meet 50 MHz. Timing failures and timeouts are outcomes of the declared matrix, not missing work; missing/corrupt jobs would be reported by `make check-hardware-v2`.

A2 candidate stream (`results/v2/raw/intervals/a2_candidate_stream.json`, standalone pipeline harness): 4096 back-to-back tokens with acceptance spacing [1, 1], retirement spacing [1, 1], visible latency [22, 22], transfer [23, 23], occupancy max 23 (mean 22.866); under 20 % bubbles / 30 % stalls: acceptance spacing [1, 41], occupancy mean 11.538. Request interval (`a2-cache-d1-p0-l1.json`): 200/200 measured pairs equal the inferred interval, R(N) = N + 31.
...
|---|---:|---:|---:|---:|
| 1 (`a1-cache-d1-p0-l1`) | 4,030 | 2,220 | 342 | — |
| 2 (`a1-cache-d1-p0-l2`) | 7,345 | 3,738 | 642 | +1,518 |
| 4 (`a1-cache-d1-p0-l4`) | 14,035 | 6,773 | 1,225 | +4,553 |

Cycles on the common corpus: `45·⌈N/L⌉ + 7 + L` when every candidate is legal (11 cycles per illegal candidate); Σ speed-up 1.91× for two lanes and 3.36× for four (v1 records `results/decisions/a1-cache-d1-p0-l{1,2}_native_1000.csv`, U13 record under `results/v2/raw/decisions/a1-cache-d1-p0-l4/`). Development routes of the four-lane core: seed 1 `route_timeout` at 1,200 s and 3,600 s, seed 2 met 50 MHz at 65.96 MHz (`docs/lanes.md` §5).

## 3. Coefficient quantization ladder (U14; `docs/precision_v2.md`)

Common-state sensitivity, development split (`benchmarks/states/corpus_d1.jsonl`, 1000 states; `benchmarks/states/corpus_d1_upgrade_dev.jsonl`, 2000 states) — planning data, not a final evaluation:
...
| P0 with constant multiplies, DSP allowed (reference only) | 25 | 18 | 17 | — | — | — | DSP 4 |
...
Every median is reached within the horizon; restricted means are means to the cap (an estimate of the restricted mean, not of unbounded survival). The development pilot (seeds 10000–10019, `results/v2/summary/analysis_pilot.json`, `analysis_pilot50k.json`) is reported in `docs/quality_v2.md` §4 and was used only to size the study.
...
Timing journal of the A2 development routes (worst paths before/after the P14 regrouping, 71.77 → 76.36 MHz at the 50 MHz constraint, seed 1): `docs/timing_journal.md`. Replay traces validated by `tools/check_trace.py`: `docs/replay.md`.
