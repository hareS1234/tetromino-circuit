...
This matrix links each A2 promise to its check. Counts belong to individual checks and can share
inputs. The 1,000-state v1 corpus feeds V11, V13, and the interval run. V04 sees the same 250 mixed
boards twice. Command logs live under
...
| proof | compactor equivalence | **unbounded** k-induction (boolector, depth 2, ~51 s) of `line_clear_parallel` against an independent running-index filter plus rank order/uniqueness, keep, count and zero-padding properties; extreme cases covered | `make formal-compactor`, `results/formal/compactor.json` |
...
| proof | best reduction | **unbounded** k-induction: dominance over an arbitrary witness token, provenance (best only changes to the accepted token), monotonicity, clear; covers reached | `make formal-a2-control`, `results/formal/reducer.json` |
| proof | control conservation | **unbounded** k-induction on the abstract 5-bank pipeline with the production advance discipline: in-flight count equals set valid bits (≤ banks), output serial equals the next to retire (no spontaneous or duplicate retirement), stability under stalls, flush on reset; fill and drain covered | `results/formal/control.json`, `results/formal/control_cover.json` |
...
| regression | A0/A1 coverage | directed RTL tests, A0 100-state corpus, two-lane A1 200 states, depth-two A1 100 states, cache, lanes and lookahead tests after the top-level change | `make regress-a0-a1` |
...
The three formal checks are unbounded k-induction results from the pinned SBY/boolector toolchain.
They cover the combinational compactor, the reducer, and the abstract control discipline. Full-core
equivalence rests on simulation in V11–V14. The proofs allow `m_ready` to vary freely. They make no
eventual-completion claim under a permanent stall. Bounded covers show that the extreme cases are
reachable.
