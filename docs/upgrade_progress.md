...
* Last passing job: U05 (U03 is blocked on the remote run only; its local validation passed).
* Current job: U06.
* Active process/log: none.
* Next command: `rtl/line_clear_pipe.sv` (P4–P12 under `advance_i`), ready/valid wrapper, `tb/tb_clear_pipe.py`, native stream harness; gate `make test-compactor-pipe`, `make test-compactor-native MODE=stream`, `make synth-compactor-pipe`.
