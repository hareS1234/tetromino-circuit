...
// Registered result one cycle after start.  Subtractions are signed seven-bit so a height
// smaller than the bottom offset cannot wrap.
module drop_fast (
    input  logic        clk,
    input  logic        rst,
    input  logic        start_i,
    ...
    always_comb begin
        geom_ok = shape_valid_i && ({1'b0, x_i} + {2'b00, width_i} <= 5'd10);
        for (int k = 0; k < 4; k++) begin
            ...
            d[k]    = colmask_i[k] ? ($signed({2'b00, hsel[k]}) - $signed({5'd0, bottom_i[2*k +: 2]})) : -7'sd1;
        end
        m = d[0];
        for (int k = 1; k < 4; k++)
            if (d[k] > m) m = d[k];
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            done_o <= 1'b0; legal_o <= 1'b0; y_o <= 5'd0; phys_y_o <= 6'd0;
        end else begin
            done_o <= start_i;
            if (start_i) begin
                legal_o  <= geom_ok && ({1'b0, y_land} + {4'd0, height_i} <= 7'd20);
                y_o      <= (geom_ok && ({1'b0, y_land} + {4'd0, height_i} <= 7'd20)) ? y_land[4:0] : 5'd0;
                phys_y_o <= geom_ok ? y_land : 6'd20;
            end
        end
    end
endmodule
